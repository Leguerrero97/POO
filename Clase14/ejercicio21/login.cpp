...
Login::Login(QWidget *parent) : QWidget(parent), adminDB(new AdminDB(this)),
                               intentosFallidos(0), imagenActualizada(false) {
    ...
    // Conectar señal de validación exitosa (dentro del constructor de Login)
    connect(adminDB, &AdminDB::signal_usuarioValidado, this, &Login::on_usuarioValidado);
...
void Login::handleLogin() {
    ...
    QString tabla = "usuarios"; // <-- Nombre de tu tabla de usuarios

    QStringList datosPersonales = adminDB->validarUsuario(tabla, username, password);

    if (!datosPersonales.isEmpty()) {
        emit signal_usuarioValidado(username); // Emitir señal de éxito
    } else {
        intentosFallidos++;
        if (intentosFallidos >= 3) {
            // Bloquear el botón de login
            loginButton->setEnabled(false);

            // Guardar el estado de bloqueo en QSettings
            QString configPath = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation) + "/config.ini";
            QSettings settings(configPath, QSettings::IniFormat);
            settings.setValue("bloqueado", true);
            settings.setValue("tiempoBloqueo", QDateTime::currentDateTime().addSecs(60)); // Bloqueo por 60 segundos

            // Mostrar mensaje de bloqueo
            QMessageBox::warning(this, "Bloqueado", "Demasiados intentos fallidos. Intente de nuevo en 1 minuto.");

            // Iniciar temporizador para desbloquear después de 60 segundos
            QTimer::singleShot(60000, this, &Login::habilitarLogin);
        } else {
            QMessageBox::warning(this, "Error", "Usuario o contraseña incorrectos");
            passwordField->clear();

            // Registrar evento de inicio de sesión fallido
            adminDB->registrar(username); // <-- Pasar el nombre de usuario
        }
    }
}
...
void Login::on_usuarioValidado(QString usuario) {
    emit signal_usuarioValidado(usuario); // Emite la señal al Manager
    close(); // Cierra el formulario de Login
}
